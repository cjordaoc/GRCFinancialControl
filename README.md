...
- **ETC parsing.** `EtcExcelParser` title-cases employee names, captures employee identifiers, normalizes levels, extracts projected margin %, ETC age, remaining weeks, and status with resilient numeric parsing, honors Resourcing layouts where headers span multiple rows (row 3 headers, row 5 data), skips subtotal rows such as "Result," and resolves engagement IDs from the `ETC INFO` sheet when the Resourcing `Activity` column omits `E-#######` tokens.
