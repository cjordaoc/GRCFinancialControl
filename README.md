...
- **Modal dialog configuration** is centralized in `ModalDialogOptions`:
  - Tokenized sizing: `DialogContentRatioCompact` (0.7), `DialogContentRatioStandard` (0.85), `DialogContentRatioWide` (0.92) control dialog dimensions as percentage of owner window size.
  - Layout modes: `CenteredOverlay` or `OwnerAligned`.
  - Behavior flags: `ShowWindowControls` (system chrome), `DimBackground` (overlay), `FreezeOwner` (disable owner during modal display).
  - Title overrides and container margins for fine-tuned presentation.
  - All resource lookups enforce fail-fast validation; missing theme keys throw exceptions.
...
- Shared behaviors: `NumericInputNullSafety` (attached property in `GRC.Shared.UI/Behaviors`) prevents binding exceptions on empty numeric inputs by forcing zero on focus loss.
- Keep new UI surfaces declarative and reuse the shared controls/templates/converters/behaviors before introducing app-specific XAML.
...
- The planner's Save action opens the latest plan for the selected engagement (or starts a new one) and renders invoice lines in a tabular editor where confirmed items remain visible but read-only.
- Each invoice line now surfaces a read-only **Due Date** column that recalculates _Emission date + Payment terms_ to the next Monday, keeping controllers aligned with payment windows without leaving the grid.
...
- Invoice lines auto-generate emission dates from the first emission date and payment terms, rebalance editable percentages/amounts to keep totals at 100% of the engagement value, display all monetary fields with the engagement currency symbol, and retain free-form additional details per line for downstream exports.
- The Notes tab stores plan-level additional details alongside instructions so ancillary information is persisted and available to notifications and reviewers.
...
2. **Full Management Data Import:**
   ...
   - Excel columns map to database fields:
    ...
    - "TER FYTD" → `RevenueToDateValue` (falls back to backlog-based calculation when the column is missing)
...
- The GRC Team workspace provides a unified interface for managing PAPDs and Managers through a tabbed view, allowing administrators to add, edit, view, and delete team members alongside their engagement assignments.
