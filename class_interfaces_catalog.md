...
| Class | `App.Presentation.Converters` | `BoolToThemeResourceBrushConverter` | Maps boolean flags to theme-aware brushes via resource key lookup. | `Convert`, `ConvertBack`, `TrueResourceKey`, `FalseResourceKey` | Avalonia `Application.Resources`, `IBrush` | Stable | Dynamically resolves brushes from theme resources for validation highlighting. |
| Class | `App.Presentation.Converters` | `PercentageOfSizeConverter` | Multiplies size values by percentage factors for proportional layout bindings. | `Convert`, `TryParsePercentage` | Avalonia sizing | Stable | Shared across both apps for responsive sizing (MaxWidth=85%). Consolidated from 3 duplicates. |
