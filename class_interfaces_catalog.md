...
| Interface | `GRCFinancialControl.Persistence.Services.Interfaces` | `IRetainTemplateGenerator` | Generates Retain upload templates based on allocation data. | `GenerateRetainTemplateAsync` | ClosedXML, `ILogger` | Stable | Produces Excel copies alongside input workbook. |
